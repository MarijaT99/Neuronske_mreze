...
% Naziv sekcije ključnih reči na srpskom (IEEEtran podrazumevano piše "Index Terms").
...
poseban model po vrsti prepoznaje bolest - sa \emph{flat} pristupom koji
...
sporednu. Najbolji flat model (ResNet-50) postiže makro F1 od $0{,}9843$ na test
...
što je nepopravljiv izvor greške koji flat model nema. Konačno, kroz Grad-CAM
...
implementiramo \emph{flat} klasifikator sa 38 klasa, te eksplicitno poredimo dva
pristupa.

Glavni doprinosi rada su: (1) implementacija i poređenje flat i hijerarhijskog
...
\begin{itemize}
  \item \textbf{Baseline CNN} treniran od nule (4 konvolucijska bloka sa batch
...
regularizacijom težina), uz kosinusni raspored stope učenja, rano zaustavljanje
...
\subsection{Flat modeli}
Na validacionom skupu, baseline CNN (od nule) postiže makro F1 od $0{,}9826$, a
...
\subsection{Flat pristup naspram hijerarhije}
Tabela~\ref{tab:main} i Slika~\ref{fig:flatvshier} prikazuju glavni rezultat na
\emph{test} skupu. Flat ResNet-50 postiže makro F1 od $0{,}9843$, dok
hijerarhija postiže $0{,}9741$ - razlika je $-0{,}0102$ u korist flat
pristupa. Isti trend važi i za ostale metrike.

\begin{table}[!htbp]
  \caption{Flat naspram hijerarhijskog pristupa (test skup). Makro F1 je
  primarna metrika; tačnost je sporedna.}
  \label{tab:main}
  \centering
  \begin{tabular}{lccc}
    \toprule
    Metrika & Flat & Hijerarhija & $\Delta$ \\
    \midrule
...
\begin{figure}[!htbp]
  ...
  \caption{Flat naspram hijerarhijskog pristupa na test skupu.}
...
prvim nivoom (Slika~\ref{fig:errprop}). Flat model nema ovaj kaskadni izvor
...
Matrica konfuzije flat modela (Slika~\ref{fig:cm}) pokazuje izraženu dijagonalu
...
\begin{figure}[!htbp]
  ...
  \caption{Matrica konfuzije flat ResNet-50 modela (test, normalizovano po
...
hijerarhija blago zaostaje. Razlog je dvojak: (1) flat model već postiže
...
grešaka. Flat model zajednički modeluje (vrsta, bolest) i izbegava kaskadu.
...
\begin{figure}[!htbp]
  ...
  \caption{Grad-CAM mape flat modela: gde model ``gleda''. Toplota je pretežno
...
\section{Zaključak}
Uporedili smo flat i hijerarhijski pristup klasifikaciji biljnih bolesti na
PlantVillage skupu, uz evaluaciju prilagođenu neuravnoteženim podacima. Flat
