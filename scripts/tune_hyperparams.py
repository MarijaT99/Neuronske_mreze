"""Optuna podešavanje hiperparametara za flat eksperiment (optimizuje val macro F1).

Pretražuje learning rate, weight decay, dropout, jačinu augmentation i imbalance
loss. Svaki trial trenira kratko (nekoliko epoch-a) i prijavljuje najbolji
validacioni macro F1 — primarnu metriku projekta (NE accuracy). Predviđeno za
pokretanje na Kaggle-u.
...
def main() -> int:
    ...
    def objective(trial: "optuna.Trial") -> float:
        ...
        cfg = load_config(resolve_path(args.config))  # nova kopija po trial-u
        d = cfg.to_dict()
        ...
        trainer.fit()
        return trainer.best_score  # najbolji val macro F1
